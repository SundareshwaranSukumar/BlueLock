...
const SYSTEM = `You are BlueLock Concierge — the on-stadium AI for M. Chinnaswamy.
...
Gates: A=Raghavendra/North, B=Pavilion/East, C=Garden/West, D=Metro/South.
...
function extractJSONTail(text: string): { reply: string; meta: { suggestedAction: "REDIRECT" | "STAY" | "PROCEED"; targetGate: "A" | "B" | "C" | "D" | null } } {
  const match = text.match(/\{[\s\S]*\}\s*$/);
  let meta: { suggestedAction: "REDIRECT" | "STAY" | "PROCEED"; targetGate: "A" | "B" | "C" | "D" | null } = { suggestedAction: "PROCEED", targetGate: null };
  let reply = text;
  if (match) {
    try {
      const parsed = JSON.parse(match[0]);
      if (parsed.suggestedAction) meta = { suggestedAction: parsed.suggestedAction, targetGate: parsed.targetGate ?? null };
    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        try {
          ...
          if (aiRes.status === 429) {
            return new Response(JSON.stringify({ replyText: "Concierge is rate-limited. Try again shortly.", suggestedAction: "STAY" }), {
              status: 200, headers: { "Content-Type": "application/json", ...cors },
            });
          }
          if (aiRes.status === 402) {
