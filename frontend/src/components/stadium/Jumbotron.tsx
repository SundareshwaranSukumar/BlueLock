...
export function Jumbotron() {
  ...
  const rr =
    match.overs !== "0.0" ? (match.runs / Math.max(0.1, parseFloat(match.overs))).toFixed(2) : "—";

  return (
    <div className="hex-frame rounded-md overflow-hidden">
      <p className="font-hud text-[9px] tracking-[0.35em] text-center text-muted-foreground py-1">
        {VENUE_LABEL}
      </p>
...
function Cell({
  ...
  const color =
    accent === "lsg" ? "var(--color-cyan)" : accent === "pbks" ? "var(--color-warn)" : undefined;
  return (
    <div className="bg-card/80 px-4 py-3">
      <p className="font-hud text-[10px] tracking-[0.3em] text-muted-foreground">{label}</p>
      <p
        className={hud ? "font-hud text-2xl font-bold mt-0.5" : "mt-0.5"}
