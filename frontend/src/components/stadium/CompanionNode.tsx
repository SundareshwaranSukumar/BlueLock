          ...
          {(["CSK", "MI"] as const).map((t) => (
            <button
              ...
              style={
                team === t
                  ? {
                      background: t === "CSK" ? "var(--color-csk)" : "var(--color-mi)",
...
function Companion({ name, team, line }: { name: string; team: "CSK" | "MI"; line: string }) {
  ...
  return (
    <div className="flex items-start gap-3">
      <div
        className="relative w-12 h-12 shrink-0 hex-clip flex items-center justify-center font-hud text-sm font-bold"
        style={{ background: color, color: "var(--color-background)" }}
      >
        {name[0]}
      </div>
      <div className="hex-frame rounded-md px-3 py-2 flex-1 relative">
        <p className="font-hud text-[10px] tracking-[0.25em]" style={{ color }}>
