...
export function BookingConfirmation({ onBookAnother }: Props) {
  ...
  const loadColor =
    ticket.metroLoad === "crit" ? "var(--color-crit)" :
    ticket.metroLoad === "warn" ? "var(--color-warn)" : "var(--color-ok)";

  return (
    <div className="max-w-3xl mx-auto space-y-5">
      <header className="text-center space-y-2">
        <p className="font-hud text-[11px] tracking-[0.4em] text-cyan">// BOOKING CONFIRMED</p>
        <h2 className="font-display text-3xl sm:text-4xl font-bold">You're in, {userName || "Fan"}.</h2>
        <p className="text-muted-foreground text-sm">CSK vs MI · M. Chinnaswamy Stadium · Tonight 19:30</p>
      </header>
        ...
        <div className="flex flex-col sm:flex-row">
          <div className="flex-1 p-5 space-y-4">
            <div className="flex items-start justify-between">
              <div>
                <p className="font-hud text-[10px] tracking-[0.3em] text-muted-foreground">TICKET ID</p>
              ...
              <span
                ...
                style={{ background: team === "CSK" ? "var(--color-csk)" : "var(--color-mi)", color: "var(--color-background)" }}
              >
                {team} SUPPORTER
              </span>
            </div>
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">{ticket.recommendedRoute}</p>
          </div>
            ...
            <div className="hex-frame rounded-sm w-32 h-32 flex items-center justify-center">
              <div className="grid grid-cols-8 gap-[2px]">
                {Array.from({ length: 64 }).map((_, i) => (
                  <span key={i} className="w-2 h-2" style={{ background: (i * 37) % 3 ? "var(--color-foreground)" : "transparent" }} />
                ))}
              </div>
            </div>
            <p className="font-hud text-[10px] tracking-[0.3em] text-muted-foreground">SCAN AT GATE</p>
      ...
      <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
        <button
          onClick={() => { clearTicket(); onBookAnother(); }}
...
function InfoCard({ title, line1, tag, tagColor, hint }: { title: string; line1: string; tag: string; tagColor: string; hint: string }) {
  return (
    <div className="hex-frame rounded-md p-4 space-y-2">
      <div className="flex items-center justify-between">
        <p className="font-hud text-[10px] tracking-[0.3em] text-muted-foreground">{title.toUpperCase()}</p>
        <span className="font-hud text-[9px] tracking-[0.3em] px-1.5 py-0.5 rounded-sm" style={{ background: tagColor, color: "var(--color-background)" }}>{tag}</span>
      </div>
