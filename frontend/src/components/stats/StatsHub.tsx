      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Macro view · LSG vs PBKS anchor · Live when API keys set
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          {SCHEDULE.map((m) => (
            <div key={m.id} className="hex-frame rounded-md p-4">
              <p className="font-hud text-[10px] tracking-[0.25em] text-cyan">
                {m.date.toUpperCase()} · {m.time}
              </p>
              <p className="font-display text-xl font-bold mt-2">
                {m.home} <span className="text-muted-foreground text-sm">vs</span> {m.away}
              </p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {PLAYERS.map((p) => (
            <button
              key={p.id}
              onClick={() => setOpen(p)}
              className="hex-frame rounded-md p-4 text-left hover:glow-cyan transition"
            >
              <p className="font-hud text-[10px] tracking-[0.25em] text-cyan">
                {p.team} · {p.role}
              </p>
          ...
          <HexFrame tone="cyan" className="w-[90%] max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <div className="bg-card p-6 rounded-sm">
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  onClick={() => setOpen(null)}
                  className="font-hud text-xs tracking-[0.3em] text-muted-foreground hover:text-foreground"
                >
                  CLOSE ✕
                </button>
              </div>
              <div className="grid md:grid-cols-2 gap-6 mt-5">
                ...
                <div>
                  <p className="font-hud text-[10px] tracking-[0.3em] text-muted-foreground mb-2">
