  ...
  return (
    <div className="hex-frame rounded-md p-4 h-full flex flex-col">
      <div className="flex items-center justify-between mb-3">
        <p className="font-hud text-xs tracking-[0.3em] text-cyan">
          // DIRECTOR RADIO · OPS TERMINAL
        </p>
        <span className="flex items-center gap-1.5 font-hud text-[10px] tracking-[0.25em] text-ok">
          <span className="w-1.5 h-1.5 rounded-full bg-[color:var(--color-ok)] animate-pulse" />{" "}
          LIVE
        </span>
      </div>
      <div
        ref={ref}
        className="flex-1 min-h-[200px] max-h-[320px] overflow-y-auto bg-background/60 border border-border rounded-sm p-3 font-mono text-xs space-y-2"
      >
        {directives.length === 0 && (
          <p className="text-muted-foreground">
            // Awaiting directive. Trigger a Bypass Order from any critical gate.
          </p>
        )}
        {directives.map((d) => (
