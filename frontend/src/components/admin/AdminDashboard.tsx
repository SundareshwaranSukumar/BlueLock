  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <Kpi label="STADIUM FILL" value={`${stadiumFill}%`} accent="var(--color-cyan)" sub={`${totalLoad}/${totalCapacity} cap units`} />
        <Kpi label="INFLOW RATE" value={`${totalFlow}/min`} accent="var(--color-ok)" sub="Aggregate of all gates" />
        <Kpi label="CRITICAL GATES" value={String(critCount)} accent={critCount ? "var(--color-crit)" : "var(--color-ok)"} sub={critCount ? "Bypass required" : "All within bounds"} />
        <Kpi label="SCORE" value={`${match.runs}/${match.wickets}`} accent="var(--color-warn)" sub={`${match.overs} ov · WP ${match.winProbability}`} />
      </div>
    ...
    <div className="hex-frame rounded-md p-4">
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-muted-foreground text-xs mt-0.5">Aggregate fan density across concourses</p>
        </div>
        <div className="flex items-center gap-2 font-hud text-[9px] tracking-[0.25em]">
          <span>LOW</span>
          <span className="w-24 h-2 rounded-full" style={{ background: "linear-gradient(90deg,var(--color-ok),var(--color-warn),var(--color-crit))" }} />
          <span>HIGH</span>
        </div>
      </div>
      <div
        ...
        {Array.from({ length: ROWS * COLS }).map((_, i) => {
          ...
          return (
            <div
              ...
              style={{ background: `oklch(${0.35 + v * 0.3} ${0.13 + v * 0.1} ${hue} / ${0.25 + v * 0.75})` }}
        ...
        {STANDS.map((s) => {
          ...
          return (
            <div key={s.name} className="text-center">
              <p className="font-hud text-[9px] tracking-[0.25em] text-muted-foreground">{s.name.toUpperCase()}</p>
              <p className="font-hud text-sm font-bold" style={{ color: g.status === "CRITICAL" ? "var(--color-crit)" : g.status === "WARNING" ? "var(--color-warn)" : "var(--color-ok)" }}>
                {Math.round(g.load)}%
              </p>
            </div>
          );
  ...
  return (
    <div className="hex-frame rounded-md p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <p className="font-hud text-[10px] tracking-[0.3em] text-muted-foreground">{free.toLocaleString()} SPACES FREE</p>
      </div>
          ...
          return (
            <div key={p.id} className="hex-frame rounded-sm p-3">
              <div className="flex items-center justify-between font-display">
                <span className="font-semibold">{p.id} · {p.name}</span>
                <span className="font-hud text-xs" style={{ color }}>{p.capacity - p.filled} FREE</span>
              </div>
              <div className="mt-2 h-1.5 rounded-full bg-muted overflow-hidden">
                <div className="h-full transition-all" style={{ width: `${pct}%`, background: color, boxShadow: `0 0 8px ${color}` }} />
              </div>
              <div className="flex justify-between font-hud text-[10px] tracking-[0.25em] text-muted-foreground mt-1.5">
                ...
                <span>{p.filled}/{p.capacity} · {pct}%</span>
              </div>
            </div>
          );
        })}
